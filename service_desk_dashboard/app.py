...
from utils import (
    load_data, apply_filters, calculate_kpis, calculate_monthly_trends,
...
def load_cached_data():
    """Carga datos con cache para mejorar rendimiento."""
    tickets_path = "data/tickets.csv"
    actions_path = "data/action_items.csv"

    if not os.path.exists(tickets_path) or not os.path.exists(actions_path):
        st.error("  Archivos de datos no encontrados. Por favor ejecuta `python data_generator.py` primero.")
        st.stop()

    return load_data(tickets_path, actions_path)
